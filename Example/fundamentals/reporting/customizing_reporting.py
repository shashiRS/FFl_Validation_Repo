...
class CustomTestcaseReport(CustomReportTestCase):
    ...
    def on_result(self, pr_details: ProcessingResult, ts_result: TeststepResult):
        _log.debug(f"processing input: {ts_result.test_input.reference.name}")

        m_res = ts_result.measured_result
        e_res = ts_result.teststep_definition.expected_results[None]
        (
            _,
            status,
        ...
        df = {
            "Recording name": ts_result.test_input.reference.name,
            "Measured result": ts_result.measured_result.value_numerator,
            "Verdict": verdict,
        }
        self.results = pd.concat([self.results, pd.DataFrame([df])], ignore_index=True)

    def overview(self):
        s = "<h3>You can also add bar charts like the one below</h3>"
        s += self.results.to_html()
