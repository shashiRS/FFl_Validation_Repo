...
from utilities.signal_definitions import ExampleSignals, ExampleSignals2, ExampleSignalsCompliance
...
def generate_bsig_compliance(bsig: Path, bsig_other: Path = None, splits=None):
    """Generate a BSIG for the example signal definition."""
    # timestamps are in microseconds (us) sampling as unit64 data type.
    if bsig:
        bsig = Path(bsig)
    ...
    ecu_a = _make_binary_signal(N)
    ecu_b = _make_binary_signal(N)
    sil_a = _make_binary_signal(N)
    sil_b = _make_binary_signal(N)

    some_sig_a = _make_activation_signal(N)
    some_sig_b = _make_activation_signal(N)
    some_other_sig_x = _make_activation_signal(N)
    ...
    for st_idx, et_idx in splits:
        ...
        exp_sd = ExampleSignalsCompliance()
        os.makedirs(bsig_path.parent, exist_ok=True)

        with BsigWriter(bsig_path) as wrt:
            wrt[SignalReader.MTS_TS_SIGNAL] = ts
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.ACTIVATION_A].signals[0]] = sig_a
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.ACTIVATION_B].signals[0]] = sig_b
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.BINARY_ACTIVATION_A].signals[0]] = binary_activation_a
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.BINARY_ACTIVATION_B].signals[0]] = binary_activation_b
            # wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.EGO_VELO].signals[0]] = ego_vx
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.ECU].signals[0]] = ecu_a[st_idx:et_idx]
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.SIL].signals[0]] = sil_a[st_idx:et_idx]
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.SIG_A].signals[0]] = some_sig_a
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.SIG_B].signals[0]] = some_sig_b
            wrt[exp_sd.signal_properties[ExampleSignalsCompliance.Columns.SIG_X].signals[0]] = some_other_sig_x
